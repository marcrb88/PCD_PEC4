...
def clean_and_homogenize(df_perf, df_aband):
    """
    Reanomenem les columnes del dataset taxa_abandonament.xlsx perquè coincideixi amb el dataset rendiment_estudiants.xlsx.
...
def aggregate_by_branch(df, metric_col):
    """
    Agrupem totes les files per les característiques demanades i calculem la mitjana de la mètrica en ambdós datasets.
    ...
    # Generem la nova columna amb el rendiment o abandonament mitjà i resetejem l'índex per obtenir el dataframe.
...
def merge_datasets(df_perf, df_aband):
    """
    Fusionem ambdós datasets en un. El dataset resultant només contindrà les files coincidents en ambdós datasets.
